/**
 * pdf-generator-v5.js
 * 100% Reliable PDF Generator for ElyHelper
 * Converts any image format (WEBP, PNG, JPG, BMP) into standardized PNG streams,
 * embeds AcroForm pushbuttons with interactive reveal/hide scripts.
 */

const ElyPDFGenerator = {
  /**
   * Converts any image format (including WEBP) to standard PNG data URL via canvas
   * This prevents "Error: SOI not found in JPEG" when uploading WEBP or PNG images.
   */
  async convertToStandardPng(imageSrc, width, height) {
  ...
  async createInteractivePDF({ imageSrc, naturalWidth, naturalHeight, masks, maskStyle }) {
    ...
    // Standardize page dimensions (max 1400pt)
    ...
    // ALWAYS convert to standard PNG to guarantee 100% compatibility with WEBP, JPG, etc.
    const safePng = await this.convertToStandardPng(imageSrc, naturalWidth, naturalHeight);
    const embeddedImage = await pdfDoc.embedPng(safePng);
    ...
    masks.forEach((mask, index) => {
      ...
      const fieldName = `mask_btn_${index}`;
